...
/**
 * Created by Gogent on 2015/10/2.
 */
public class ComputerPlayer {
    ...
    private ChessStatus[] chessStatus = new ChessStatus[4]; //表示四个方向的棋子到达边界
    ...
    private int getValue(int r, int c, ChessType chessType) {
        int[] dir = new int[4];
        dir[0] = this.getHorCount(r, c, chessType); //横向棋子个数
        dir[1] = this.getVerCount(r, c, chessType); //纵向棋子个数
        dir[2] = this.getSloRCount(r, c, chessType);//反斜向棋子个数
        dir[3] = this.getSloLCount(r, c, chessType);//斜向棋子个数
    ...
    private int getHorCount(int r, int c, ChessType chessType) {
        ...
        for (int j = c + 1; j < c + 5; j++) {
            ...
            if (chessMap[r][j] == chessType) {
                ...
                chessStatus[0] = (chessMap[r][j] == ChessType.NONE) ? ChessStatus.ALIVE : ChessStatus.DIED;
                t1 = j;
                break;
            }
        }
    ...
    private int getVerCount(int r, int c, ChessType chessType) {
        ...
        for (int i = r + 1; i < r + 5; i++) {
            ...
            if (chessMap[i][c] == chessType) {
                ...
                chessStatus[1] = (chessMap[i][c] == ChessType.NONE) ? ChessStatus.ALIVE : ChessStatus.DIED;
                t1 = i;
                break;
            }
        }
                ...
                if (chessStatus[2] == ChessStatus.DIED) {
                    ...
                    if (chessStatus[2] == ChessStatus.ALIVE) {
                        ...
                        for (int p = tr1 + 1, q = tc1 + 1; p < tr1 + 5; p++, q++) {
                            if (p >= DrawView.ROWS || q >= DrawView.COLS) {
                                break;
                            }
